...
describe("GiuPay Contracts", function () {
  let shopRegistry: any;
  let paymentEscrow: any;
  let warrantySBT: any;
  let usdc: any;
  let owner: any, bot: any, feeWallet: any, shop: any, buyer: any, stranger: any;
  ...
  describe("ShopRegistry", function () {
    it("shop da duyet -> isVerified = true", async function () {
      expect(await shopRegistry.isVerified(shop.address)).to.equal(true);
    });

    it("dia chi chua dang ky -> isVerified = false", async function () {
      expect(await shopRegistry.isVerified(buyer.address)).to.equal(false);
    });

    it("dang ky shop phat event ShopRegistered", async function () {
      await expect(shopRegistry.registerShop(stranger.address, ethers.id("d2")))
    ...
    it("dang ky trung dia chi -> revert", async function () {
      await expect(
        shopRegistry.registerShop(shop.address, ethers.id("again"))
      ).to.be.revertedWith("Already registered");
    });

    it("chi owner duoc dang ky shop", async function () {
      await expect(
        shopRegistry.connect(stranger).registerShop(stranger.address, ethers.id("d"))
      ).to.be.revertedWithCustomError(shopRegistry, "OwnableUnauthorizedAccount");
    });

    it("duyet shop chua ton tai -> revert", async function () {
      await expect(shopRegistry.verifyShop(stranger.address)).to.be.revertedWith("Not found");
    });

    it("tu choi shop -> isVerified quay ve false", async function () {
      await shopRegistry.rejectShop(shop.address);
      expect(await shopRegistry.isVerified(shop.address)).to.equal(false);
    });

    it("getShopCount dem dung so shop", async function () {
      expect(await shopRegistry.getShopCount()).to.equal(1n);
      await shopRegistry.registerShop(stranger.address, ethers.id("d3"));
      expect(await shopRegistry.getShopCount()).to.equal(2n);
    });
  });
  ...
  describe("PaymentEscrow - thanh toan", function () {
    it("thanh toan thanh cong + tach phi 0.1%", async function () {
      ...
      expect(escrow.amount).to.equal(NET);
      expect(escrow.status).to.equal(0);
      expect(escrow.buyer).to.equal(buyer.address);
      expect(escrow.shop).to.equal(shop.address);
      expect(await usdc.balanceOf(feeWallet.address)).to.equal(FEE);
    });

    it("phat event PaymentReceived", async function () {
      const deadline = await futureDeadline();
      await expect(
        paymentEscrow.connect(buyer).pay(ORDER_ID, TX_HASH, shop.address, AMOUNT, deadline)
      )
    ...
    it("chong replay: cung txHash lan 2 -> revert", async function () {
      await payOrder();
      await expect(payOrder()).to.be.revertedWith("Already processed");
    });

    it("trung orderId (txHash khac) -> revert Order exists", async function () {
      await payOrder();
      await expect(payOrder(ORDER_ID, ethers.id("TX-002"))).to.be.revertedWith("Order exists");
    });

    it("giao dich het han -> revert", async function () {
      const past = (await time.latest()) - 1;
      await expect(
        paymentEscrow.connect(buyer).pay(ORDER_ID, TX_HASH, shop.address, AMOUNT, past)
    ...
    it("so tien = 0 -> revert Invalid amount", async function () {
      const deadline = await futureDeadline();
      await expect(
        paymentEscrow.connect(buyer).pay(ORDER_ID, TX_HASH, shop.address, 0, deadline)
    ...
    it("vuot tran 100000 USDC -> revert Invalid amount", async function () {
      ...
      const deadline = await futureDeadline();
      await expect(
        paymentEscrow.connect(buyer).pay(ORDER_ID, TX_HASH, shop.address, tooBig, deadline)
      ).to.be.revertedWith("Invalid amount");
    });
    ...
    it("shop chua verified -> revert Shop not verified", async function () {
      const deadline = await futureDeadline();
      await expect(
        paymentEscrow.connect(buyer).pay(ORDER_ID, TX_HASH, stranger.address, AMOUNT, deadline)
      ).to.be.revertedWith("Shop not verified");
    });
  });
  ...
  describe("PaymentEscrow - release", function () {
    it("bot release sau 14 ngay -> shop nhan tien", async function () {
      ...
      await expect(paymentEscrow.connect(bot).releaseEscrow(ORDER_ID))
        .to.emit(paymentEscrow, "EscrowReleased")
        .withArgs(ORDER_ID, shop.address, NET);
      expect(await usdc.balanceOf(shop.address)).to.equal(NET);
    });

    it("release truoc 14 ngay -> revert Too early", async function () {
      await payOrder();
      await expect(paymentEscrow.connect(bot).releaseEscrow(ORDER_ID)).to.be.revertedWith("Too early");
    });
    ...
    it("khong phai bot goi release -> revert Not bot", async function () {
      ...
      await time.increase(14 * 24 * 3600);
      await expect(
        paymentEscrow.connect(stranger).releaseEscrow(ORDER_ID)
      ).to.be.revertedWith("Not bot");
    });
    ...
    it("release 2 lan -> lan 2 revert Not active", async function () {
      ...
      await expect(paymentEscrow.connect(bot).releaseEscrow(ORDER_ID)).to.be.revertedWith("Not active");
    });

    it("release don khong ton tai -> revert Not found", async function () {
      await expect(
        paymentEscrow.connect(bot).releaseEscrow(ethers.id("NOPE"))
      ).to.be.revertedWith("Not found");
    });

    it("emergencyRelease: shop tu rut sau 16 ngay", async function () {
      await payOrder();
      await time.increase(16 * 24 * 3600);
    ...
    it("emergencyRelease truoc 16 ngay -> revert Too early", async function () {
      await payOrder();
      await time.increase(14 * 24 * 3600);
      await expect(paymentEscrow.connect(shop).emergencyRelease(ORDER_ID)).to.be.revertedWith("Too early");
    });
    ...
    it("emergencyRelease boi nguoi khong phai shop -> revert Not shop", async function () {
      ...
      await time.increase(16 * 24 * 3600);
      await expect(
        paymentEscrow.connect(stranger).emergencyRelease(ORDER_ID)
  ...
  describe("PaymentEscrow - tranh chap", function () {
    it("buyer mo tranh chap -> status Disputed", async function () {
      await payOrder();
      await expect(paymentEscrow.connect(buyer).openDispute(ORDER_ID))
    ...
    it("khong phai buyer mo tranh chap -> revert Not buyer", async function () {
      await payOrder();
      await expect(paymentEscrow.connect(stranger).openDispute(ORDER_ID)).to.be.revertedWith("Not buyer");
    });
    ...
    it("shop hoan tien khi tranh chap -> buyer nhan lai tien", async function () {
      ...
      expect(await usdc.balanceOf(buyer.address)).to.equal(before + NET);
      const e = await paymentEscrow.getEscrow(ORDER_ID);
    ...
    it("refundByShop khi khong co tranh chap -> revert No dispute", async function () {
      await payOrder();
      await expect(paymentEscrow.connect(shop).refundByShop(ORDER_ID)).to.be.revertedWith("No dispute");
    });
    ...
    it("khong phai owner goi adminResolve -> revert", async function () {
      ...
      await paymentEscrow.connect(buyer).openDispute(ORDER_ID);
      await expect(
        paymentEscrow.connect(stranger).adminResolve(ORDER_ID, true)
  ...
  describe("PaymentEscrow - cau hinh", function () {
    it("setFeeWallet cap nhat + phat event", async function () {
      await expect(paymentEscrow.connect(owner).setFeeWallet(stranger.address))
        .to.emit(paymentEscrow, "FeeWalletUpdated")
        .withArgs(stranger.address);
      expect(await paymentEscrow.feeWallet()).to.equal(stranger.address);
    });

    it("setFeeWallet dia chi 0 -> revert", async function () {
      await expect(
        paymentEscrow.connect(owner).setFeeWallet(ethers.ZeroAddress)
      ).to.be.revertedWith("Zero address");
    });

    it("setBotAddress chi owner", async function () {
      await expect(
        paymentEscrow.connect(stranger).setBotAddress(stranger.address)
      ).to.be.revertedWithCustomError(paymentEscrow, "OwnableUnauthorizedAccount");
    });
  });
  ...
  describe("WarrantySBT", function () {
    ...
    it("mint thanh cong -> buyer co 1 SBT", async function () {
      await expect(mintSBT())
        .to.emit(warrantySBT, "SBTMinted")
        .withArgs(1, buyer.address, ORDER_ID);
      expect(await warrantySBT.balanceOf(buyer.address)).to.equal(1n);
      expect(await warrantySBT.ownerOf(1)).to.equal(buyer.address);
    });

    it("luu dung du lieu SBT + map orderId -> tokenId", async function () {
      await mintSBT();
      const data = await warrantySBT.getSBTData(1);
      expect(data.productName).to.equal("iPhone 15");
      expect(data.imageCid).to.equal(IMG_CID);
      expect(await warrantySBT.orderToToken(ORDER_ID)).to.equal(1n);
    });

    it("chi owner duoc mint", async function () {
      await expect(
        warrantySBT.connect(stranger).mint(buyer.address, ORDER_ID, "x", IMG_CID, META_CID, 365)
      ).to.be.revertedWithCustomError(warrantySBT, "OwnableUnauthorizedAccount");
    });

    it("mint trung orderId -> revert Already minted", async function () {
      await mintSBT();
      await expect(mintSBT()).to.be.revertedWith("Already minted");
    });

    it("SBT khong chuyen duoc - transferFrom revert", async function () {
      await mintSBT();
      await expect(
        warrantySBT.connect(buyer).transferFrom(buyer.address, shop.address, 1)
    ...
    it("SBT khong chuyen duoc - safeTransferFrom revert", async function () {
      await mintSBT();
      await expect(
        warrantySBT.connect(buyer)["safeTransferFrom(address,address,uint256,bytes)"](
          buyer.address, shop.address, 1, "0x"
        )
      ).to.be.revertedWith("SBT: non-transferable");
    });
    ...
    it("bao hanh con han -> true, het han -> false", async function () {
      ...
      expect(await warrantySBT.isWarrantyValid(1)).to.equal(false);
    });

    it("bot burn SBT sau khi het han", async function () {
      await mintSBT(30);
      await time.increase(31 * 24 * 3600);
      await expect(warrantySBT.connect(bot).burn(1))
        .to.emit(warrantySBT, "SBTBurned")
        .withArgs(1, ORDER_ID);
      expect(await warrantySBT.balanceOf(buyer.address)).to.equal(0n);
      expect(await warrantySBT.orderToToken(ORDER_ID)).to.equal(0n);
    });

    it("burn truoc khi het han -> revert Not expired yet", async function () {
      await mintSBT(30);
      await expect(warrantySBT.connect(bot).burn(1)).to.be.revertedWith("Not expired yet");
    });

    it("khong phai bot goi burn -> revert Not bot", async function () {
      await mintSBT(30);
      await time.increase(31 * 24 * 3600);
      await expect(warrantySBT.connect(stranger).burn(1)).to.be.revertedWith("Not bot");
    });

    it("setBotAddress dia chi 0 -> revert", async function () {
      await expect(
        warrantySBT.connect(owner).setBotAddress(ethers.ZeroAddress)
      ).to.be.revertedWith("Zero address");
    });
  });
  ...
  describe("PaymentEscrow - confirmReceived (fix bug khoa tien)", function () {
    it("sau openDispute + confirmReceived, status quay ve Active + dispute dong", async function () {
      ...
      expect(e.disputeClosed).to.equal(true);
    });
    ...
    it("tranh chap DANG MO (chua confirm) van khong release duoc (status Disputed)", async function () {
      ...
      await time.increase(14 * 24 * 3600);
      await expect(
        paymentEscrow.connect(bot).releaseEscrow(ORDER_ID)
