...
const BATCH_BLOCKS = Number(process.env.READ_BATCH_BLOCKS ?? 2_000);
const LOOKBACK      = Number(process.env.READ_LOOKBACK_BLOCKS ?? 50_000);
...
function fmtUsdc(raw: bigint): string {
  return `${ethers.formatUnits(raw, 6)} USDC`; // USDC ERC-20 interface trên Arc dùng 6 decimals
}

function assertConfig() {
  if (!ARC_RPC_URL) throw new Error("Thiếu ARC_RPC_URL trong backend/.env");
  if (!ESCROW_ADDRESS) throw new Error("Thiếu ESCROW_CONTRACT_ADDRESS trong backend/.env");
}

async function printEscrowDetail(contract: Contract, orderId: string) {
  try {
...
async function readByTxHash(provider: JsonRpcProvider, contract: Contract, txHash: string) {
  console.log(`\n🔍 Đọc giao dịch: ${txHash}\n`);

  const receipt = await provider.getTransactionReceipt(txHash);
  if (!receipt) { console.log("❌ Không tìm thấy giao dịch này trên Arc Testnet."); return; }
  ...
  const iface = new Interface(ESCROW_ABI as any);
  let found = false;
  ...
  for (const log of receipt.logs) {
    ...
    try {
      ...
      if (parsed.name === "PaymentReceived") {
        ...
        console.log(`   args: ${JSON.stringify(parsed.args.map((a: any) => a?.toString?.() ?? a))}`);
      }
...
async function scanRecent(provider: JsonRpcProvider, contract: Contract) {
  ...
  for (let start = from; start <= head; start += BATCH_BLOCKS) {
    ...
    for (const ev of disputed as ethers.EventLog[]) {
      totalEvents++;
...
async function main() {
  ...
  if (TX_HASH) await readByTxHash(provider, contract, TX_HASH);
  else await scanRecent(provider, contract);
}
